  ...
  public class HandPool :
    ...
    */
    ...
    public void SetGroup(string groupName){
		StartCoroutine(setGroup(groupName));			
    }

		private IEnumerator setGroup(string groupName)
		{
			yield return new WaitForEndOfFrame();
			...
			for (int i = 0; i < ModelPool.Count; i++)
			{
				ModelGroup g = ModelPool[i];
				if (g.GroupName == groupName && g.IsEnabled)
				{

				}
				else if (g.GroupName == "PhyHand" && !g.IsEnabled){
					EnableGroup(g.GroupName);
					g.IsEnabled = true;
				}
				else if (g.GroupName == groupName && !g.IsEnabled)
				{
					EnableGroup(g.GroupName);
					g.IsEnabled = true;
				}
				else if (g.GroupName != groupName && g.IsEnabled && g.GroupName != "PhyHand")
				{					
					DisableGroup(g.GroupName);
					g.IsEnabled = false;
				}
			}
		}
